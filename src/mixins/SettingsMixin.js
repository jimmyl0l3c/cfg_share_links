...
export default {
	...
	methods: {
		...
		async getLabelMode() {
			...
			return settings && settings.defaultLabelMode && settings.defaultLabelMode >= 0 && settings.defaultLabelMode <= 2 ? settings.defaultLabelMode : 0
		},
		async getCustomLabel() {
			const settings = await this.getSettings()
			return settings && settings.defaultLabel ? settings.defaultLabel : 'Custom link'
		},
		async getMinTokenLength() {
			const settings = await this.getSettings()
			return settings && settings.minTokenLength ? settings.minTokenLength.toString() : '3'
		},
